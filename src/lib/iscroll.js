...
(function (window, doc) {
  var m = Math, _bindArr = [],
    ...
    iScroll = function (el, options) {
      ...
      // 初始化之后检查一次高度的变化, 应对在编译阶段初始化iscroll的情况
      that.checkOnce = that.options.checkOnce || false
  ...
  iScroll.prototype = {
    ...
    initInput: function () {
      ...
      for (var i = 0; i < len; i++) {
        item = inputs[i]
        if (/email|number|password|search|tel|text|url/.test(item.type) && !item.disabled) {
          item.obj = this
          item.addEventListener('focus', function () {
            that.input = item
            that.oldWinHeight = window.innerHeight
          }, false)
    ...
    _checkDOMChanges: function () {
      var sizeNotChange = this.scrollerW == this.scroller.offsetWidth * this.scale && this.scrollerH == this.scroller.offsetHeight * this.scale
      if (this.moved || this.animating || sizeNotChange) return

      if (this.checkOnce && !sizeNotChange) {
    ...
    inputToShow: function () {
      ...
      var viewWinH = this.oldWinHeight - window.innerHeight
      var inputTop = this.input.getBoundingClientRect().top
      var inputH = this.input.clientHeight
      var diff = viewWinH - inputTop - inputH
      if (diff < 0 && viewWinH > 0) {
        this.scrollTo(0, Math.abs(diff), 0, true)
      }
    },
    ...
    _pos: function (x, y) {
      ...
      this.x = x
      this.y = y
      if (this.options.onScrollChange) this.options.onScrollChange.call(this, x, y)
    },
